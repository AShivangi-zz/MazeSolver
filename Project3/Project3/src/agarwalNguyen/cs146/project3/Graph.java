...
public class Graph {
	...
	public void dfs()
	{
		...
		getVertex(0,0).lab = count;//starting point is 0
		int VisitedCells = 1;
		...
		while (VisitedCells < TotalCells)
		{
			...
			if (!neighbors.isEmpty()) //have to check if visited
			{
				...
				if (neighbor.west == CurrentCell.east) //neighbor is left
				{
					CurrentCell.east = false;
					neighbor.west = false;
					CelltoPush = map[CurrentCell.x - 1][CurrentCell.y];
					getVertex(CurrentCell.x - 1,CurrentCell.y).lab = (char) (count +1);//count while solving
				}
				else if (neighbor.south == CurrentCell.north) //neighbor is north
				{
					CurrentCell.north = false;
					neighbor.south = false;
					CelltoPush = map[CurrentCell.x][CurrentCell.y - 1];
					getVertex(CurrentCell.x,CurrentCell.y -1).lab = (char) (count +1);//count while solving
				}
				else if (neighbor.east == CurrentCell.west) //neighbor is right
				{
